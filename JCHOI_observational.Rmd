...
### Step 2**: Descriptive statistics. (create Table 1 & spaghetti plots)
### Step 3**: Fitting LMMs
...
```{r}
#Summarizing the number of repeated measurement for each person (median; quantile)
nrepeat<-data.frame()
...
for(i in unique(data)){
  ...
  nrepeat[i,9]<-tmpdata$valuen2[tmpdata$time0==sort(tmpdata$time0[tmpdata$time0 >nrepeat[i,10]], decreasing = F)[1]][1]
  }
...
nrepeat<-subset(nrepeat, measurement != 0 & aftertime0 != 0 )
nrepeat
```
...
```{r}
print('number of repeated measurement (median/iqr)')
median(nrepeat$measurement); quantile(nrepeat$measurement, probs=c(0.25, 0.75))
print(' number of repeated measurement after prescription (median/iqr)')
median(nrepeat$aftertime0, na.rm = TRUE); quantile(nrepeat$aftertime0, probs=c(0.25, 0.75), na.rm = TRUE)
print('2nd last measurement timing before the prescription (median/iqr)')
median(nrepeat$`t-1last`, na.rm = TRUE); quantile(nrepeat$`t-1last`, probs=c(0.25, 0.75), na.rm = TRUE)
print('Last measurement timing before the prescription (median/iqr)')
median(nrepeat$tlast, na.rm = TRUE); quantile(nrepeat$tlast, probs=c(0.25, 0.75), na.rm = TRUE)

#HbA1c
print('HbA1c: 2nd last before the prescription (mean/sd)')
mean(nrepeat$`hb_t-1last`, na.rm=T); sd(nrepeat$`hb_t-1last`, na.rm=T)
print('HbA1c: Last before the prescription (mean/sd)')
mean(nrepeat$hb_tlast, na.rm=T); sd(nrepeat$hb_tlast, na.rm=T)


#Glucose
print('Glucose: 2nd last before the prescription')
mean(nrepeat$`glu_t-1last`, na.rm=T); sd(nrepeat$`glu_t-1last`, na.rm=T)
print('Glucose: Last before the prescription')
mean(nrepeat$glu_tlast, na.rm=T); sd(nrepeat$glu_tlast, na.rm=T)

median(nrepeat$tfirst, na.rm = TRUE); quantile(nrepeat$tfirst, probs=c(0.25, 0.75), na.rm = TRUE)
mean(nrepeat$glu_tfirst, na.rm=T); sd(nrepeat$glu_tfirst, na.rm=T)


```


### 2.1 Make a baseline table
```{r}
#table1(~sexe+ 
#        leeftijd +
#        etnwhite +
...
### 2.2 Box plots: values at eath time point (in month)
...
### 4.1 Cleaning up dosage data
Categorize dosage into numeric values
```{r}

med$dperdag<-med$dosage
med$dperdag<-ifelse(is.na(med$dperdag), med$dosering, med$dperdag)


#0.5 per dag
half<-
c("1D0.5T",
  "1D0.5T (dosering verlaagd)")                                                       
...
c("'s avonds 1 tablet",
  ...
  "1 maal per dag 1 tablet.", 
  "1D-",
  "1D1", 
  "1D1IJ", 
  "1D1T",
  "1D1T  bij  ontbijt per eerstvolgende baxter",
  "1D1T  Bij eerstvolgende  Baxter bij avondeten",
  "1D1T  bij het avondeten innemen",
  "1D1T  bij het avondeten innemen,",
  "1D1T  bij het avondeten innemen, eerst voor 14 dag",
  "1D1T  herhaalservice",
  "1D1T  medicatie gestaakt", 
  "1D1T  verlaging dosering",
  "1D1T AV",
  "1D1T BO",
  "1D1T controle via specialist",
  "1D1T MO",
  "1D1T MO TM",
  "1D1T MO VM",
  "1D1T TM",
  ...
  "1x per dag 1 tablet",
  "1x per dag 1 tablet-*--*-",
  "2 keer per dag een halve tablet-*--*-",
  "1x per dag een halve tablet-*--*-", 
  "1x per dag een halve tablet",
  "2D0.5T  in baxter toevoegen", 
  "2x per dag een halve tablet", 
  "2x per dag een halve tablet-*--*-",
  "1x per dag 25 eenheden",
  "1D1IJ  SOLOSTARPEN 1 doos",
  "1d1t - start nadat glimepiride op is",
  " 1d1t - start nadat glimepiride op is",
  "ochtend: 1 tablet-*--*-",
...
onenhalf<-
c("1D1.5T MO",
  "1x per dag anderhalve tablet",
  "1x per dag anderhalve tablet-*--*-",
  "3 keer per dag een halve tablet-*--*-",
  "3x per dag een halve tablet-*--*-"
  )
...
c("2D1T",
  " 2D1T",
  "2D1T   Baxter",
  "2D1T  Bij eerstvolgende  Baxter aanpassen",
  "2D1T  bij het avondeten innemen, eerst voor 14 dag",
  "2D1T  dosering verhoogd.",
  "2D1T  dosis aanpassing per eerstvolgende baxter",
  "2D1T  herhaal service",
  "2D1T herhaalservice",
  "2D1T  herhaalservice",
  "2D1T  in volgende baxter",
  "2D1T  is opgehoogd",
  "'s morgens 1 en 's avonds 2-*--*-",
  "'s morgens 1 en 's avonds 2 tabletten-*--*-",
  "'s morgens 2 tabletten-*--*-",
  "'s morgens en voor de nacht 1 tablet",
  "2D1T", 
  "1 keer per dag 2 tabletten-*--*-", 
  "2D1T herhaalservice",
  " 2D1T herhaalservice",
  "1D2T",
  "1D2T  eerste twee weken 1 tablet",
  "1D2T MO",
  "1D2T MO  (opgehoogd van 1,5 naar 2)",
  "1D2T VM MO", 
  "1x per dag 2 tabletten",
  "1x per dag 2 tabletten-*--*-",
  "2 keer per dag 1 tablet-*--*-",
  "2 maal per dag 1 tablet",
  "2 maal per dag 1 tablet.",
  "2 maal per dag 1 tablet. TIJDENS of vlak NA het eten innemen.",
  "2 maal per dag 1 tablet.\nTIJDENS of vlak NA het eten innemen.",
  "2D1T  na 2 weken 3d1t",
  "2D1T  start",
  "2D1T  verhoogd",
  "2D1T 2 per dag 1 tablet", 
  "2D1T MO TM",
  "2D1T TM",
  "2D1T TO",
  "2x per dag",
...
#3 per dag
three<-
c("'s morgens 2 en 's avonds 1-*--*-", 
  "'s morgens 2 en 's avonds 1 tablet-*--*-",
  "'s morgens 3 tabletten-*--*-",
  "1 keer per dag 3 tabletten-*--*-", 
  "1D3T",
  "1D3T  dosis gewijzigd",
  "1D3T MO  (opgehoogd van 2 naar 3)", 
  "2 keer per dag anderhalve tablet-*--*-",
  "1x per dag 3 tabletten", 
  "1x per dag 3 tabletten-*--*-", 
  "2x per dag anderhalve tablet-*--*-", 
  "3 keer per dag 1-*--*-", 
  "3 maal per dag 1 tablet.", 
  "3 maal per dag 1 tablet. TIJDENS of vlak NA het eten innemen.",
  "3 maal per dag 1 tablet.\nTIJDENS of vlak NA het eten innemen",
  "3 maal per dag 1 tablet.\nTIJDENS of vlak NA het eten innemen.",
  "3 keer per dag 1 tablet-*--*-",
  "3D1T",
  "3D1T  ontbijt 1- avondeten 2",
  "3D1T MO TM",
  "3x per dag-*--*-",
  "3x per dag 1-*--*-",
  "3x per dag 1 tablet",
  "3x per dag 1 tablet-*--*-",
  "3x per dag 8 eenheden",
  "3d",
  " 3d",
  "ochtend: 2-*-avond: 1-*-",
  "ochtend: 1 tablet-*-avond: 2 tabletten-*-",
  "ochtend: 3 tabletten-*--*-",
  "3D1T per 6/11 dosis verhoogd!",
  " 3D1T per 6/11 dosis verhoogd!",
  ";1-0-2 tbl",
  "ochtend: 1-*-avond: 2-*-",
  "3 maal per dag 1 tablet.\nTIJDENS of vlak NA het eten innemen."
...
#4 per dag
four<-
c("'s morgens 1, 's middags 1 en 's avonds 2-*--*-",
  "'s morgens 2, 's middags 1 en 's avonds 1-*--*-", 
  "'s morgens 4 tabletten-*--*-",
  "1 keer per dag 4 tabletten-*--*-",
  "2 keer per dag 2 tabletten-*--*-",
  "1x per dag 4 tabletten-*--*-",
  "2D2T",
  "2D2T geleidelijk ophogen",
  "2x per dag 2-*--*-",
  "2x per dag 2 tabletten",
  "2x per dag 2 tabletten-*--*-", 
  "4 keer per dag 1 tablet-*--*-",
  "4D1T", 
  "4x per dag 1 tablet", 
  "4x per dag 1 tablet-*--*-",
  "ochtend: 1-*-middag: 1-*-avond: 2",
  "ochtend: 4 tabletten-*--*-",
  ";2dd2"
  )             
...
#5 per dag
five<-
c("'s morgens 2, 's middags 1 en 's avonds 2-*--*-",
  "5 keer per dag 1 tablet-*--*-", 
  "5D1T",
  "5x per dag 1 tablet-*--*-",
  "avond: 1 tablet-*--*-" ,
  "ochtend: 2-*-middag: 1-*-avond: 2"
)               
                                                   
                                                    
#6 per dag
six<-
c(";bij de maaltijd 6 eenheden",
  "3 keer per dag 2 tabletten-*--*-",
  "3x per dag 2 tabletten-*--*-"
)

#Unclear (more than once_ or insulin injection
unknown<-
c(" ",
  "- -  GVH",
  "- -  GVSCH",
  ...
  "GVSCH  10 pennen", 
  "- -  GVSCH  10 pennen",
  "1 maal daags 1 injectie",
  "  1 maal daags 1 injectie",
  "bij avondeten SoloStarpen",
  "  bij avondeten SoloStarpen",
  "over op lantus ipv Toujeo",
  "  over op lantus ipv Toujeo",
  "STOP in volgende baxter",
  "  STOP in volgende baxter",
  "volgens voorschrift met pennaaldjes en naaldenco",
  "  volgens voorschrift met pennaaldjes en naaldenco",
  "1d16EH volgens afspraak",
  " 1d16EH volgens afspraak",
  "1d16EH volgens afspraak (Toujeo)",
  " 1d16EH volgens afspraak (Toujeo)", 
  "1d16EH volgens afspraak (Toujeo)  3 doosjes, morg",
  " 1d16EH volgens afspraak (Toujeo)  3 doosjes, morg",
  "; 1x per dag starten met 10 eh. naast novorapid",
  ";2x50 EH", 
  "1D60EH",
  "1x per dag 1,8 milligram injecteren" 
  )
                                   

med$dperdag<-med$dosage
med$dperdag[c(which(med$dosage %in% half))]<-0.5
med$dperdag[c(which(med$dosage %in% one))]<-1
med$dperdag[c(which(med$dosage %in% onenhalf))]<-1.5
med$dperdag[c(which(med$dosage %in% two))]<-2
med$dperdag[c(which(med$dosage %in% three))]<-3
med$dperdag[c(which(med$dosage %in% four))]<-4
med$dperdag[c(which(med$dosage %in% five))]<-5
med$dperdag[c(which(med$dosage %in% six))]<-6
med$dperdag[c(which(med$dosage %in% unknown))]<-NA

med$dperdag<-as.numeric(med$dperdag)
table(med$dperdag)

```


Clean up the number of prescribed medication unit
```{r}
med$number<-ifelse(is.na(med$number), med$dnumber, med$number)
med$number[med$etiket=="270 ST METFORM HCL ACT 500MG TABL\nMetformine 500mg tablet\n"]<-270
med$number[med$etiket=="180 ST METFORMINE HCL MYLAN 500 TB\nMetformine 500mg tablet\n"]<-180
med$number[med$etiket=="180 ST METFORMINE HCL ACTA 500MG T\nMetformine 500mg tablet\n"]<-180
med$number[med$etiket=="60 ST METFORMINE HCL ACTA 500MG T\nMetformine 500mg tablet\n"]<-60
med$number[med$etiket=="180 ST METFORM HCL ACT 500MG TABL\nMetformine 500mg tablet\n"]<-180
med$number[med$etiket=="270 ST METFORMINE HCL ACTA 500MG T\nMetformine 500mg tablet\n"]<-270
...
for sensitivity data set: Remove last measurements before the medication prescription to avoid random high.
```{r}
#First remove observations without before prescription data
removeid<-as.numeric()
for(i in unique(compdat_sensi$PP187ID)){
removeid[length(removeid)+1] <-ifelse(sum(compdat_sensi$time0 <=0 & compdat_sensi$PP187ID==i)==0,
                               i,
                              NA)
}
removeid<-removeid[!is.na(removeid)]
compdat_sensi<-compdat_sensi[-which(compdat_sensi$PP187ID %in% removeid), ]
...
for(i in unique(compdat_sensi$PP187ID)){
  #return the row numbers that contain the last observations before the medication prescription
  removerow[length(removerow)+1] <- which(compdat_sensi$time0==sort(compdat_sensi$time0[compdat_sensi$time0 <=0 & compdat_sensi$PP187ID==i], decreasing = T)[1] & compdat_sensi$PP187ID==i)[1]
  removerow[length(removerow)+1] <- which(compdat_sensi$time0==sort(compdat_sensi$time0[compdat_sensi$time0 <=0 & compdat_sensi$PP187ID==i], decreasing = T)[1] & compdat_sensi$PP187ID==i)[2]

  }
removerow<-removerow[!is.na(removerow)]
compdat_sensi<-compdat_sensi[-removerow, ]
